# PRD: Bankie Developer Portal (v1.3 — FX Rate Engine + Security Hardening)
...
**Phase 1.1 outcomes (delivered)**: Dashboard scopes-granted metric (unique scope count), API request logging middleware to `portal.api_logs`, audit logging for key operations (create/rotate/revoke), dashboard recent activity feed.

**Phase 1.2 outcomes (delivered)**: FX Rate Engine with CoinGecko (crypto) + ExchangeRate API (fiat) providers, USD normalization on transactions, settlement CSV with FX columns, portal USD Value column + USD Equivalent balance summary. Security hardening: Secure cookies (env-aware), JWT removed from response body, tenant_id removed from org creation, login rate limiting (5 attempts / 15min lockout), API key cache invalidation on revoke/rotate.
...
| P4 | No API usage visibility — no request logs, no rate limiting metrics | Cannot diagnose issues, no abuse protection | **High** | **Resolved** (Phase 1.1): API request logging middleware + dashboard 24h call count. Phase 2: rate limiter wiring. |
...
4. **API observability** — searchable request logs, rate limiting with standard headers ✅ (Phase 1.1: request logging + dashboard metrics delivered; Phase 2: rate limiter wiring)
5. **FX rate normalization** — USD normalization on all transactions for multi-currency reporting ✅
6. **Team collaboration** — RBAC with Owner/Admin/Member roles ⚠️ (Auth done, invite flow pending)
7. **Environment isolation** — sandbox vs production ❌ (Future)
...
- **Addressed**: Key rotation with 24h grace period, owner/admin/member RBAC, audit logging + activity feed
- **Remaining**: Member invite flow

### Persona 3: Operations Manager ("Ops Olivia")
- **Role**: Ops person monitoring transaction flow and reconciliation
- **Needs**: Dashboard overview, transaction monitoring, webhook delivery status, FX visibility
- **Addressed**: Dashboard with stats, Accounts/Transactions/Reports pages proxying Core data, USD Value column on transactions, FX-enriched settlement CSV, API request logging
- **Remaining**: Webhook delivery status, API request log viewer page
...
graph TB
    ...
    subgraph "Gateway: Logging"
        AL["API Logger Middleware<br/>async write to portal.api_logs"]
        AUL["Audit Logger<br/>key create/rotate/revoke"]
    end

    subgraph "Bankie Core"
        CORE_AUTH["JWT Auth Middleware"]
        CMD["Command Channel<br/>mpsc bounded 10k"]
        BA["BankAccount Aggregate"]
        LG["Ledger Aggregate"]
        OB["Outbox Pattern"]
        FX["FX Rate Engine<br/>CoinGecko + ExchangeRate API<br/>Redis-cached"]
...
├── crates/
│   ├── bankie-core/            # Banking engine + FX Rate Engine
...
└── db/migrations/
    ...
    └── 20260301000001_fx_rate_columns.sql     # fx_rate_to_usd, amount_usd, fx_rate_source on transactions
```
...
| **FX Rate Engine in Core** | USD normalization at transaction creation time. CoinGecko (crypto) + ExchangeRate API (fiat) providers with Redis caching (60s crypto, 1h fiat). Graceful degradation: FX fields null if providers fail. | ✅ Built |
| **Security: env-aware Secure cookies** | `Secure` flag on cookies conditionally set when `ENV != local`. Eliminates R7 for production. | ✅ Built |
| **Login rate limiting** | 5 attempts per email, 15min lockout via Redis INCR + TTL. Returns HTTP 429 + Retry-After. Fail-open on Redis error. | ✅ Built |
| **API key cache invalidation** | Revoke/rotate operations delete Redis cache entry (`gw:api_key:{hash}`), ensuring stale keys are not served from cache. | ✅ Built |
...
| M8 | **Dashboard — Home/Overview** | ✅ Done | Org name, environment, total/active key counts, unique scopes-granted count, real 24h API request count from `portal.api_logs`, recent activity feed from `portal.audit_logs` |
...
| S4 | **API Request Logging** | ✅ Done | `api_logger` middleware logs all API proxy requests (method, path, status_code, latency_ms, client_ip) to partitioned `portal.api_logs` via async `tokio::spawn`. Dashboard shows real 24h call count. |
...
| S8 | **Dashboard — Transactions page** | ✅ Done | Proxies Core `/v1/transaction` with full filter support. USD Value column with FX rate subtitle. USD Equivalent balance summary for mixed-currency accounts. |
| S9 | **Dashboard — Reports page** | ✅ Done | Proxies Core settlement CSV. FX columns added: `amount_usd`, `fx_rate_to_usd`, `fx_rate_source`. |
...
|----|------------|--------|
| C1 | **Audit Log** | ✅ Done | Write logic for key ops (`api_key.created`, `api_key.rotated`, `api_key.revoked`). Dashboard recent activity feed via `GET /portal/v1/dashboard/activity`. |
...
| **Security** | Portal session | httpOnly cookie, SameSite=Lax, Secure (non-local env), CSRF token in header | ✅ |
| **Security** | Login rate limiting | 5 attempts / 15min lockout per email, HTTP 429 + Retry-After | ✅ |
| **Security** | JWT not in response body | Token delivered only via httpOnly cookie, never in JSON response | ✅ |
| **Security** | API key cache invalidation | Redis cache entry deleted on revoke/rotate | ✅ |
...
| **Data retention** | API request logs | Partitioned by month (Feb-Apr 2026 created) | ✅ Schema + writes |
| **FX** | USD normalization | CoinGecko (crypto: 60s cache) + ExchangeRate API (fiat: 1h cache). Graceful degradation. | ✅ |
...
### FX Rate Columns on `public.transactions`

```sql
-- Migration: 20260301000001_fx_rate_columns.sql
ALTER TABLE transactions
    ADD COLUMN fx_rate_to_usd DECIMAL(24,12),
    ADD COLUMN amount_usd     DECIMAL(19,2),
    ADD COLUMN fx_rate_source  VARCHAR(50);
CREATE INDEX idx_transactions_amount_usd ON transactions(amount_usd);
```

FX conversion happens at transaction creation time via `FxRateService`. Fields are `NULL` if FX providers are unavailable (graceful degradation).
...
```
Note: JWT is delivered **only** via httpOnly cookie, never in the response body (H2 security hardening).

Cookies set: `portal_session=<jwt>; HttpOnly; SameSite=Lax; Max-Age=86400[; Secure]` + `csrf_token=<random32>; SameSite=Lax; Max-Age=86400[; Secure]` (`Secure` flag added when `ENV != local`)
...
| Method | Path | Auth | Description |
|--------|------|------|-------------|
| GET | `/portal/v1/dashboard/stats` | Session | Org stats: key counts, unique scopes granted, 24h API call count |
| GET | `/portal/v1/dashboard/activity` | Session | Recent 10 audit log entries for the org |
...
|-------|-----------|--------|
| Signup | Email + password (Argon2id) → session JWT + CSRF cookies (no tenant_id in request — H3) | ✅ |
| Login | Email + password verify → session JWT + CSRF cookies. Rate limited: 5 attempts / 15min lockout (M1). | ✅ |
| Session JWT | httpOnly cookie, SameSite=Lax, Secure (non-local env — H1), 24h TTL, contains: sub, org_id, tenant_id, role, csrf | ✅ |
| Auth response | JWT delivered only via httpOnly cookie, never in JSON body (H2) | ✅ |
...
| Cache invalidation | Revoke/rotate deletes Redis cache entry (`gw:api_key:{hash}`) — prevents stale key serving (M4) | ✅ |
...
| Session hijacking | Spoofing | httpOnly + SameSite + Secure cookies, CSRF token, no JWT in body | ✅ |
...
| Log data exfiltration | Info Disclosure | API + audit logs written; no PII redaction yet | ⚠️ Future (PII redaction) |
| Brute-force login | DoS | 5 attempts / 15min lockout per email via Redis | ✅ |
...
**Total Unit Tests: 251** (9 common + 128 core + 114 gateway)

### Phase 1.1: Dashboard Polish + Logging ✅ COMPLETE (PR #13)

**Delivered**:
- Dashboard scopes-granted now computes unique scope count across active API keys (was incorrectly showing `total_api_keys`)
- `api_logger` middleware logs all API proxy requests to partitioned `portal.api_logs` via async `tokio::spawn`
- Dashboard 24h API call count reads from `portal.api_logs` (was hardcoded to 0)
- Audit logging for key ops: `api_key.created`, `api_key.rotated`, `api_key.revoked` written to `portal.audit_logs`
- Dashboard recent activity feed via `GET /portal/v1/dashboard/activity` (was static placeholder)

### Phase 1.2: FX Rate Engine + Security Hardening ✅ COMPLETE (PR #15, #16)

**Delivered — FX Rate Engine (PR #16)**:
- `FxRateService` with pluggable provider architecture (`FxRateProvider` trait)
- `CoinGeckoProvider` — crypto rates (BTC, ETH, USDT → USD) with 60s Redis cache
- `ExchangeRateProvider` — fiat rates (TWD → USD via `open.er-api.com`) with 1h Redis cache
- USD normalization at transaction creation time: `fx_rate_to_usd`, `amount_usd`, `fx_rate_source` columns on `transactions` table
- Graceful degradation: FX fields are `NULL` if all providers fail (never blocks transaction)
- Settlement CSV: 3 new columns (`amount_usd`, `fx_rate_to_usd`, `fx_rate_source`)
- Portal SPA: USD Value column on Transactions page with FX rate subtitle
- Portal SPA: USD Equivalent balance summary when accounts span multiple currencies
- `MockProvider` for deterministic testing

**Delivered — Security Hardening (PR #15)**:

| ID | Fix | Detail |
|----|-----|--------|
| H1 | **Secure cookies** | `Secure` flag added conditionally when `ENV != local`. Applied to session + CSRF cookies on login, signup, logout. |
| H2 | **Token removed from body** | `AuthResponse` struct contains only `user` + `organization`. JWT delivered exclusively via httpOnly cookie. |
| H3 | **tenant_id removed from org creation** | `CreateOrgRequest` accepts only `name`. tenant_id auto-assigned via DB sequence. |
| M1 | **Login rate limiting** | 5 attempts per email, 15min lockout. Redis INCR + TTL. HTTP 429 + `Retry-After` header. Fail-open on Redis error. |
| M4 | **API key cache invalidation** | Revoke/rotate calls `DEL gw:api_key:{hash}` on Redis. Prevents stale cached keys from being served. |
...
| API request log PII redaction | 2h | Redact sensitive fields in `api_logs` |
| SPA: Webhook + Logs pages | 6h | Endpoint management, delivery history, log viewer |
| **Total** | **~26h** | |

### Phase 4: Sandbox + Remaining Hardening

| Task | Effort | Details |
|------|--------|---------|
| Sandbox environment routing | 4h | `bk_test_` keys → sandbox tenant, simulated processing |
| Full OWASP scan | 2h | Automated security scan (H1-H3 + M1 + M4 already addressed in Phase 1.2) |
...
|-------|-------|--------|--------|
| 1. Foundation | Org/auth, API keys, proxy, SPA | ~48h | ✅ Done (PR #8) |
| 1.1 Dashboard Polish | Scopes granted, API request logging, audit logging, activity feed | ~10h | ✅ Done (PR #13) |
| 1.2 FX + Security | FX Rate Engine (CoinGecko + ExchangeRate API), USD normalization, security hardening (H1-H3, M1, M4) | ~16h | ✅ Done (PR #15, #16) |
| 2. Rate Limit + Team | Wire rate limiter, member invite, grace expiry | ~14h | Next |
| 3. Webhooks | Dispatcher, signing, retry, logs | ~26h | Planned |
| 4. Hardening | Sandbox, OWASP scan, load test | ~9h | Planned |

---

## 11. Success Metrics

| Metric | Before | Target (1mo post-launch) | Current Status |
|--------|--------|--------------------------|----------------|
...
| API request log coverage | 0% | 100% of proxied calls | ✅ `api_logger` middleware logs all proxy requests |
| Audit log coverage | 0% | Key lifecycle + admin actions | ✅ Key create/rotate/revoke logged |
| FX rate coverage | 0% transactions | 100% of deposit/withdrawal transactions | ✅ USD normalization at creation time |
| Rate limit violations caught | 0 | Track and alert | ⚠️ Proxy rate limiter built, not wired. Login rate limiter active. |
| Unit test count | 230 (Phase 1) | 250+ | ✅ 251 tests (9 common + 128 core + 114 gateway) |
...
| R7 | **Session cookie not Secure flag** | Low | Medium | ~~Fixed in PR #15~~: `Secure` flag added conditionally when `ENV != local`. | ✅ Resolved |
...
| Q6 | Production cookie settings: add `Secure` + `SameSite=Strict`? | Security | ✅ Resolved — `Secure` added env-conditionally (PR #15). `SameSite=Lax` kept (Strict breaks cross-origin redirects). |
...
| Rate limiting | Per-key, published limits | Per-key | Per-key | Per-key | ⚠️ Built, not wired (login rate limiting active) |
...
| Request logs | Dashboard + API | Dashboard | Dashboard | Dashboard + API | ✅ Middleware + dashboard 24h count |
| Audit log | Yes | Yes | Yes | Yes | ✅ Key ops + activity feed |
| FX normalization | Built-in | N/A | N/A | Built-in | ✅ CoinGecko + ExchangeRate API |
...
| **FX Rate Engine** | `FxRateService` in bankie-core that fetches USD exchange rates from CoinGecko (crypto) and ExchangeRate API (fiat). Redis-cached (60s crypto, 1h fiat). Graceful degradation on failure. |
| **USD Normalization** | At transaction creation, amount is converted to USD equivalent using current FX rate. Stored as `amount_usd` + `fx_rate_to_usd` + `fx_rate_source` on the transaction row. |
