...
use crate::interest::models::{InterestAccrual, InterestRateConfig, InterestRateTier};
...
impl DatabaseClient for PgPool {
    ...
    async fn get_active_rate_config(
        ...
        let config = sqlx::query_as::<_, InterestRateConfig>(
            r#"
            SELECT id, currency, account_kind, day_count, posting_frequency, posting_day,
                   effective_from, effective_to, is_active
            FROM interest_rate_configs
            WHERE currency = $1
              AND account_kind = 'Interest'
              AND is_active = true
    ...
    async fn get_rate_tiers(&self, rate_config_id: Uuid) -> Result<Vec<InterestRateTier>, Error> {
        let tiers = sqlx::query_as::<_, InterestRateTier>(
            r#"
    ...
    async fn get_interest_eligible_accounts(&self) -> Result<Vec<BankAccountWithLedger>, Error> {
        let accounts = sqlx::query_as::<_, BankAccountWithLedger>(
            ...
            LEFT JOIN ledger_views l ON b.payload->>'ledger_id' = l.view_id
            WHERE b.payload->>'status' = 'Approved'
              AND b.payload->>'kind' = 'Interest'
              AND b.payload->>'currency' IN ('USD', 'TWD')
            "#,
        )
        .fetch_all(self)
        .await?;
