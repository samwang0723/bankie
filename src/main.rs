...
use sqlx::PgPool;
use state::{new_application_state, ApplicationState, BankAccountLoaderSaver};
...
async fn process_commands(state: ApplicationState<PgPool>, rx: Receiver<BankAccountCommand>) {
    while let Ok(command) = rx.recv() {
        ...
        let cqrs = state.bank_account.clone().unwrap().cqrs.clone();
        let _ = cqrs.execute(&id.to_string(), command).await;
    }
}
...
async fn main() {
    ...
    match args.mode.as_str() {
        ...
        "server" => {
            let mut state = new_application_state().await;
            let (tx, rx): (Sender<BankAccountCommand>, Receiver<BankAccountCommand>) =
                mpsc::channel();
            state = state.with_command_sender(tx);

            // Spawn a background thread to process commands
            let clone_state = state.clone();
            task::spawn(async move {
                process_commands(clone_state, rx).await;
            });
